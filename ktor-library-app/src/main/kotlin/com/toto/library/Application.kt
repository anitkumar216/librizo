...
fun main() {
    ...
    val mongoUri = config.property("mongodb.uri").getString()
    val client = KMongo.createClient(mongoUri)
    val database = client.getDatabase("mongodbVSCodePlaygroundDB") // replace "yourDatabaseName" with your actual database name


    val checkVersionCollection = CheckVersionCollection(database)
    //val userCollection = LibraryDetailsCollection(database)
    //val facilitiesCollection = LibraryFacilitiesCollection(database)
    //val studentDetailsCollection = StudentDetailsCollection(database)
